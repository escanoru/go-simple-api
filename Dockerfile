...
FROM golang@sha256:48c87cd759e3342fcbc4241533337141e7d8457ec33ab9660abe0a4346c30b60 AS build
...
WORKDIR /src
ADD src /src
RUN chown -R ${USERNAME} /src
USER ${USERNAME}

# download Go modules, dependencies and compile
RUN go mod tidy && go mod verify && CGO_ENABLED=0 GOOS=linux go build -o /src/go-simple-api
...
COPY --from=build /src/go-simple-api /app/go-simple-api
